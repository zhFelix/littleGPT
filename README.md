...
```
- --epochs：目标总训练轮数（若从检查点恢复，该值为总轮数，而非新增轮数）
- --batch-size：每批次的样本数（每个样本为一个长度为 block_size 的 token 序列）
- --block-size：	首选上下文长度（脚本会根据训练数据长度自动缩小，但不会超过该值）
- --learning-rate：AdamW 优化器的学习率
- --early-stopping-patience：验证损失无改善时允许的连续评估轮数，达到该值则提前终止训练。0 表示禁用早停
- --save-checkpoint-every：	每隔多少轮保存一个检查点（同时会保存最佳检查点）
- --eval-dir：手动指定验证数据目录。若未指定，脚本自动查找 ./valid 或 ./test
- --eval-every：每隔多少轮执行一次验证集评估
- --min-improvement：视为“改善”的最小验证损失下降量。只有下降超过该阈值时才重置早停计数器
